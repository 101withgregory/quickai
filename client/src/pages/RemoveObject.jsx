  ...
  return (
    <div className='flex h-full overflow-y-scroll p-6 items-start max-sm:flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} action="" className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <input onChange={(e) => setInput(e.target.files[0])} accept='image/*' className='w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border border-gray-300 text-gray-600'
          required type='file' />
        <p className='mt-6 text-sm font-medium'>Describe Object Name To Remove</p>
        <textarea onChange={(e) => setObject(e.target.value)} value={object} className='w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border border-gray-300' placeholder='e.g. , watch or spoon, Only single object name'
          required rows={4} />
